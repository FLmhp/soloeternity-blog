...
`/anime/` 读取 Bangumi 用户 `1263468` 的公开动画收藏，按想看、看过、在看、搁置和抛弃筛选。页面只读取公开数据，不需要 OAuth Token；默认仅加载“在看”，点击其他分类时再分页加载。海报卡片会限制简介行数，悬停时轻微放大，点击整张卡片可进入对应 Bangumi 条目。

### 12.3 音乐播放器

播放器使用站内托管的 APlayer `1.10.1`，避免公共 CDN 不稳定。左下角面板支持列表、随机、单曲循环三种模式，以及上一首、下一首、进度、音量和歌单切换；圆形按钮可折叠面板。
